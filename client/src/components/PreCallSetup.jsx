...
export default function PreCallSetup({ sessionData, onJoin, onBack }) {
  const [cameraMode, setCameraMode] = useState('avatar'); // 'camera' | 'avatar'
  const [cameraStream, setCameraStream] = useState(null);
  const [cameraError, setCameraError] = useState('');
  const videoRef = useRef(null);

  // Auto-detect AI persona from interview type text, override label if server provided one
  const interviewTypeText = sessionData?.materials?.interviewType || '';
  const basePersona = useMemo(() => detectPersona(interviewTypeText), [interviewTypeText]);
  const persona = sessionData?.interviewerRole
    ? { ...basePersona, label: sessionData.interviewerRole }
    : basePersona;
  ...
  return (
    <div className="precall fade-in">
      <div className="precall-container">
        <div className="precall-header-row">
          <button className="precall-back" onClick={onBack}>
          ...
          <div className="precall-header-spacer" />
        </div>
